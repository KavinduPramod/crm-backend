...
const getSMSDetails = async (req,res) => {
    try{
        ...
        const connection = await getConnection();
        const getSMSDetailsQuery = `SELECT id,phone_number,text,status,sending_date,cost,it_business_master_id,type FROM transaction_sms WHERE it_business_master_id = ? AND type = 1`;
