...
const addAccountType = async (req, res) => {
  ...
  try {
    ...
    if (itBusinessMasterInsetQueryData.affectedRows == 0) {
      ...
      const queryCM = `insert into credit_master(
                        it_business_master_id,balance,transaction_date,c_at,c_by,m_at,m_by,status) values(?,?,?,?,?,?,?,?)`;
      const resItCM = await connection.query(queryCM, [
          resItBusinessMaster.insertId.toString(),
          0.00,
          req.body.c_at,
          req.body.c_at,
          0,
